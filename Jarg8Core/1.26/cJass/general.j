...
namespace General __onload InitGeneral /* BEGIN NAMESPACE GENERAL */
    ...
    //int   hexpow                  [];
    string  hexresult               = nil;

    wchar_t Int2Hex(int i){
        int z = i;
        int m = 0;
        hexresult = nil;
        if i > 0 {
            loop{
                m = MI(z,16)
                if hexresult == nil {
                    hexresult = SubString(hexarray,m,m);
                }else{
                    hexresult = SubString(hexarray,m,m) + hexresult;
                } 
                z = z / 16;
                exif( z == 0)
            }
   ...
   wchar_t Hex2Format( wchar_t hx){
        if StringLength(hx) == 1 {
            return "0" + hx;
        }
        return hx;
   }
